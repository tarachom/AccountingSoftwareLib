    ...
    async Task ClearDeletionLabel(CancellationTokenSource cancellationToken, System.Action CallBack)
    {
        ...
        foreach (ConfigurationDirectories confDirectories in Kernel.Conf.Directories.Values)
        {
            ...
            if (recordResult.ListRow.Count > 0)
            {
                ...
                object? directoryObjectInstance = ExecutingAssembly.CreateInstance($"{NameSpageCodeGeneration}.Довідники.{confDirectories.Name}_Object");
        ...
        foreach (ConfigurationDocuments confDocuments in Kernel.Conf.Documents.Values)
        {
            ...
            if (recordResult.ListRow.Count > 0)
            {
                ...
                object? documentObjectInstance = ExecutingAssembly.CreateInstance($"{NameSpageCodeGeneration}.Документи.{confDocuments.Name}_Object");
